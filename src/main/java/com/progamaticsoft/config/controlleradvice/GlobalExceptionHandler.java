...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<?> handleEcryptionException(final EncryptionException ex, final WebRequest request) {
		final ErrorResponse error = new ErrorResponse(HttpStatus.INTERNAL_SERVER_ERROR.value(), ex.getMessage(),
				request.getDescription(false));
		return new ResponseEntity<>(error, HttpStatus.NOT_FOUND);
	}
}
